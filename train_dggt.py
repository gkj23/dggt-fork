...
def parse_args():
    ...
    parser.add_argument('--image_dir', type=str, default="/DATA_EDS2/AIGC/2312/xuhr2312/gkj/dggt-fork/data/waymo/processed/training")
...
def main(args):
    ...
    dataset = WaymoOpenDataset(args.image_dir, scene_names=[str(i).zfill(3) for i in range(0,50)], sequence_length=args.sequence_length, mode=1, views=1)
    ...
    for step in tqdm(range(args.max_epoch)):
        # 这个train script里面step代表一共进行了多少个epoch
        sampler.set_epoch(step)        
        for batch in dataloader:
            images = batch['images'].to(device)
            # print("image:",images.shape)   image: torch.Size([1, 4, 3, 350, 518]) [B,S,C,H,W]
            # sky_mask = batch['masks'].to(device).permute(0, 1, 3, 4, 2)  # [B, S, H, W, C]
            # bg_mask = (sky_mask == 0).any(dim=-1)
            sky_mask = torch.zeros_like(images).permute(0,1,3,4,2) 
            bg_mask = torch.ones(sky_mask.shape[:-1], dtype=torch.bool, device=device)
            ...
            with torch.cuda.amp.autocast(dtype=dtype):
                ...
                #dynamic mask loss
                if 'dynamic_mask' in batch:
                    print("dynamic_masks:",dynamic_masks.shape)
